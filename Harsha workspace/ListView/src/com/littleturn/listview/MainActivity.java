...
public class MainActivity extends ActionBarActivity implements ListViewListener{
	...
	public void askOpenFile(String directory) {
		// TODO Auto-generated method stub
		if(directory.endsWith(".txt") || directory.endsWith(".c")
				|| directory.endsWith(".cpp") || directory.endsWith(".java")
				|| directory.endsWith(".py") || directory.endsWith(".xml")){
			Bundle args = new Bundle();
			args.putString("directory", directory);
			TextViewFragment tvf = new TextViewFragment();
			tvf.setArguments(args);
			FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
			ft.replace(R.id.container, tvf);
			ft.addToBackStack(null);
			ft.commit();
		}
		else if(directory.endsWith(".mp3") || directory.endsWith(".m4a")){
		...
		else if(directory.endsWith(".jpg") || directory.endsWith(".png")){
